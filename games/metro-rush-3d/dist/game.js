...
function tone(type){
  ...
  const notes={smoke:[320,190,.35,.03],discovered:[1300,900,.45,.035],launch:[260,700,.3,.02],stumble:[140,95,.22,.04],caught:[180,65,.5,.04],chaseClear:[480,1080,.25,.025],escape:[550,1320,.5,.04],pace:[320,760,.25,.018],eventWarning:[460,700,.24,.025],eventStart:[520,980,.32,.03],eventProgress:[860,1420,.12,.025],stamp:[920,1450,.12,.025],eventFinish:[660,1320,.38,.04],district:[260,520,.35,.02],coin:[900+(engine.combo%8)*70,1550,.09,.025],jump:[260,590,.13,.025],slide:[340,130,.15,.025],move:[230,270,.045,.01],crash:[110,34,.4,.07],magnet:[440,1350,.38,.035],double:[660,1760,.3,.032],sneakers:[360,1080,.2,.03],jetpack:[180,800,.6,.03],board:[300,900,.23,.032],boardPickup:[660,980,.15,.03],mystery:[750,1600,.3,.033],mission:[660,1320,.4,.04],combo:[880,1760,.25,.035],stage:[440,880,.3,.03],shieldBreak:[180,80,.3,.04],land:[85,50,.08,.015],trainWarning:[140,95,.5,.035],ceiling:[140,80,.12,.02]};
...
const iconPaths={
  ...
  smoke:'<path d="M8 10h8v11H8zM10 7h4v3m-1-7c4 1-3 2 1 4M8 3c-4 1 3 2-1 4"/>',
...
function announceBonus(type,duration){
  ...
  $('notice-detail').textContent=type==='magnet'?'金币正在沿光迹飞向你':type==='board'?'抵挡一次碰撞':type==='smoke'?'烟雾遮住身后，追兵已被拉远':`${Math.ceil(duration)} 秒 · ${b.description}`;
...
function updateMenu(){
  ...
  $('difficulty-pattern').textContent={casual:'首次失误可脱险；约 7 秒无伤拉开追兵，出口 4 波考验。',classic:'约 10 秒无伤脱离贴身追赶，出口连续 5 波混合封锁。',expert:'约 13 秒无伤甩开追兵，出口 6 波车潮与封线交替。'}[difficulty];
...
function showGuide(selected='magnet'){
  if($('bonus-guide').open)return;guideResume=['running','intro','caught'].includes(engine.mode);if(guideResume)showPause();
  selectGuide(selected);$('bonus-guide').showModal();
}
function syncUI(){
  const menu=engine.mode==='menu',running=engine.mode==='running',intro=engine.mode==='intro'||engine.mode==='paused'&&engine.pausedFrom==='intro';
  $('app').dataset.state=engine.mode;
  for(const id of ['menu','controls-guide','bonus-dock'])$(id).hidden=!menu;
  for(const id of ['hud','mission-panel'])$(id).hidden=menu||intro;
  $('intro').hidden=!intro;$('skip-intro').disabled=engine.mode!=='intro';
  if(intro){const t=engine.introTime,lines=t<2.2?['凌晨 00:47 · 中央车库','就差最后一笔。','阿岚在停靠的列车旁，完成今晚的涂鸦。']:t<3.3?['手电扫过车厢','喂！站住！','巡线员老周发现了你，脚步声越来越近。']:['目标 · 北桥出口','跑！穿过封锁！','首次碰撞会被追近，贴身时再失误就会被抓。'];['intro-kicker','intro-title','intro-subtitle'].forEach((id,i)=>{if($(id).textContent!==lines[i])$(id).textContent=lines[i];});}
  $('pause').hidden=!['running','intro','caught'].includes(engine.mode);$('touch-controls').hidden=!running;$('board-action').hidden=!running;
  $('overlay').hidden=engine.mode!=='paused'&&engine.mode!=='over'&&engine.mode!=='escaped';
  ...
  const chase=engine.pursuit;$('chase-block').dataset.danger=String(chase.danger);$('chase-label').textContent=engine.mode==='caught'?'被抓住了':engine.recovery?'踉跄！巡线员追上来了':chase.danger?'贴身追赶 · 再撞会被抓':'已拉开距离 · 继续找出口';$('chase-hint').textContent=chase.danger?`无伤约 ${chase.seconds}s · 跳滑可缩短`:'失误后仍有一次脱险机会';$('chase-fill').style.transform=`scaleX(${chase.pressure/100})`;
  $('story-goal').textContent=`第 ${engine.story.leg} 程 · ${engine.story.name} ${Math.ceil(Math.max(0,engine.story.target-engine.distance))} m`;
  ...
  $('board-count').textContent=engine.boardCharges;$('board-action').disabled=!!engine.recovery||engine.board>0||engine.boardCharges===0||engine.jetpack>0||engine.landing;
  ...
  if(encounter){
    ...
    $('mission-tag').textContent=phase==='warning'?'前方事件':phase==='result'?(encounter.success?'事件完成':'未达标'):`随机事件 · ${encounter.type==='breakout'?'抵达出口':encounter.type==='rooftop'?'自选冒险':['','轻快','紧张','高压'][encounter.level]}`;
    $('mission-name').textContent=meta.name;
    $('mission-reward').textContent=`+${encounter.reward} 分`;
    $('mission-detail').textContent=phase==='result'?(encounter.success?`+${encounter.coins} 金币${encounter.streakReward?' · 三连胜补给已到手':` · 连续完成 ${engine.eventStats.streak} 次`}`:encounter.tainted?'本段发生碰撞，干净通过才算完成。':'未达标不扣分，继续探索下一段。'):encounter.type==='rooftop'?`中轨蓝色稳拿；${encounter.roofLane===-1?'左':'右'}轨上坡拿金色，每枚额外 +200 分、10 金币。`:meta.hint;
...
function startRun(){
  ...
  $('skip-intro').focus({preventScroll:true});return true;
}
function showPause(){
  if(!engine.pause())return false;$('result-eyebrow').textContent='TAKE A BREATHER';$('overlay-title').textContent='暂时歇一口气';$('overlay-message').textContent='剧情、追逐与道具计时已暂停。';$('result-stats').hidden=true;$('resume').textContent='继续奔跑 →';syncUI();$('resume').focus({preventScroll:true});return true;
}
...
function showGameOver(){
  const escaped=engine.mode==='escaped';
  ...
  $('result-eyebrow').textContent=escaped?'ESCAPE COMPLETE':'CAUGHT BY THE INSPECTOR';$('overlay-title').textContent=escaped?`抵达${engine.story.name}`:'被老周抓住了';
  $('overlay-message').textContent=escaped?`你甩开了追赶，完成第 ${engine.story.leg} 程。你可以在此收手，或穿过旧线继续冒险；巡线员仍在远处寻找你。`:{train:'从斜坡登上车顶，或提前换道。车厢间隙需要起跳越过。',oncoming:'迎面列车速度更快，看到车灯和来车提示就提前换道。',ramp:'从坡脚沿箭头进入斜坡；无法从侧面穿过坡体。',barrier:'向上跳跃，或换到空闲轨道，就能越过路障。',gate:'向下滑铲通过横杆。超级跳跃时也要留意高度。'}[engine.reason]||'贴身追赶时再次碰撞，被巡线员追上。下次用干净跳滑拉开距离。';
  ...
  $('result-reward').textContent=`脱身 ${engine.story.completed} 次 · 失误 ${engine.pursuit.hits} 次 · 最高时速 ${Math.round(engine.peakSpeed*3.6)} km/h · 完成挑战 ${engine.missionsCompleted} 项 · 滑板护身 ${engine.savedCrashes} 次 · 随机事件 ${engine.eventStats.completed} 次 · 最佳连胜 ${engine.eventStats.bestStreak} 次 · 奖励分 +${format(engine.stats.bonusPoints)}`;
  $('resume').textContent=escaped?'挑战下一片区 →':'再跑一次 ↗';renderer.shake=escaped?0:.3;syncUI();$('resume').focus({preventScroll:true});
}
function nextLeg(){if(!engine.continueRun())return false;renderer.resetCharacter();accumulator=0;syncUI();$('pause').focus({preventScroll:true});return true;}
function skipOpening(){const ok=engine.skipIntro();if(ok){events();syncUI();$('pause').focus({preventScroll:true});}return ok;}
...
function events(){
  for(const e of engine.drainEvents()){
    ...
    if(e.type==='stumble'){renderer.shake=.4;toast('脱险！追兵已贴近 · 干净跳滑拉开距离',3000);}
    if(e.type==='chaseClear')toast('拉开距离了 · 又有一次脱险机会',2400);
    if(e.type==='launch')toast('穿过封锁，抵达北桥出口！',2600);
    if(e.type==='nextLeg')toast(`继续深入旧线 · 前往${engine.story.name}，小心巡线员！`,2800);
    if(e.type==='exitMissed')toast('突围未达标 · 下一个出口继续尝试',2600);
    if(e.type==='escape')showGameOver();
    ...
    if(['magnet','double','sneakers','jetpack','board','smoke'].includes(e.type))announceBonus(e.type,e.duration);
...
document.addEventListener('keydown',event=>{
  ...
  if(engine.mode==='intro'&&['Enter',' '].includes(event.key)){event.preventDefault();if(!event.repeat)skipOpening();return;}
  ...
  if(event.key==='Escape'||event.key.toLowerCase()==='p'){event.preventDefault();if(event.repeat)return;if(['running','intro','caught'].includes(engine.mode))showPause();else if(engine.mode==='paused')resumeRun();return;}
...
$('skip-intro').addEventListener('click',skipOpening);$('start').addEventListener('click',startRun);$('pause').addEventListener('click',showPause);$('resume').addEventListener('click',()=>engine.mode==='paused'?resumeRun():engine.mode==='escaped'?nextLeg():startRun());$('home').addEventListener('click',goHome);
...
function registerTools(){
  ...
  const actions=['start','skip_intro','continue','pause','resume','left','right','jump','slide','board'];

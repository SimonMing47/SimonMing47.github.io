...
function empty(mode='classic'){const e=new RunnerEngine(91,mode);e.start({skipIntro:true});e.pursuit.pressure=88;e.obstacles=[];e.pickups=[];e.courses=[];e.tunnels=[];e.nextRow=1e9;return e;}
...
test('ground-side entry cannot teleport onto a high ramp or through a train',()=>{
  for(const type of ['ramp','train']){
    const e=empty();e.x=2.7;e.lane=1;if(type==='ramp')ramp(e,0,20,0,3.3);else train(e,0,40);e.action('left');tick(e,.3);assert.equal(e.mode,'caught');assert.ok(e.y<.2);
  }
});
test('a car gap requires a jump; its far roof catches the landing',()=>{
  for(const jump of [false,true]){
    ...
    assert.equal(e.mode,jump?'running':'caught');if(jump)assert.equal(e.surface,'roof');
  }
});
...
test('moving-roof landing checks contact time before the train arrives or its tail departs',()=>{
  const late=empty('expert');late.distance=1e5;late.y=3.4;late.vy=-16;late.grounded=false;train(late,15,26.4,0,18);late.step(.05);assert.equal(late.mode,'caught');

...
const files=['data/resources.js','data/faults.js','data/profiles.js','data/sources.js','data/groups.js','data/guide.js','data/software-init.js','data/software-sources.js',...Array.from({length:8},(_,i)=>`data/software-${i}.js`),'data/software-link.js','data/network-review.js','src/engine.js','src/models.js','src/models-v2.js','src/topology.js','src/software-diagrams.js','src/network-v31.js'];
...
const report=vm.runInContext(`(()=>{
 ...
 check(PROFILES.A5.racks===16&&PROFILES.A5.busracks===4,'A5 explicitly selected 950 product configuration');check(PROFILES.A5.sources.includes('net_a5types'),'A5 product association evidence');check(PROFILES['A5-850'].racks===0,'850 no inferred cabinet count');
 const modelFor=(id,p)=>createModel(RESOURCES.find(r=>r.id===id),p);
 const a3node=modelFor('server','A3'),a5node=modelFor('server','A5'),rack950=modelFor('rack','A5'),axes=modelFor('hccs','A5'),a850=modelFor('superpod','A5-850');
 check(a3node.parts.filter(n=>n.resource==='npu').length===16,'A3: eight paired HiAM / sixteen scheduling units');
 check(a5node.parts.filter(n=>n.resource==='npu').length===8,'950: eight processors per OS domain');
 check(rack950.parts.filter(n=>n.resource==='npu').length===64,'950: eight OS domains / 64 processors per compute rack');
 check(axes.parts.filter(n=>n.resource==='npu').length===1024,'950 logical axes: 8x8x16');
 check(a850.parts.filter(n=>n.resource==='server').length===16,'850 single-switch example: sixteen servers');
 check(a850.parts.filter(n=>n.name==='5808交换机').length===1,'850 single-switch example: one 5808');
 for(const p of ['A5','A5-850','950']){const pkg=modelFor('npu',p);check(pkg.parts.filter(n=>n.name?.startsWith('计算DIE')).length===2,p+' two compute DIE');check(pkg.parts.filter(n=>n.name?.startsWith('IO DIE')).length===2,p+' two IO DIE');}
 check(AtlasTopology.make('pod','A3').kind==='net-a3','A3 distinct topology');
 check(AtlasTopology.make('pod','A5').kind==='net-axes','950 multidimensional topology');
 check(AtlasTopology.make('pod','A5-850').kind==='net-850','850 switched topology');
 check(AtlasTopology.make('net-levels','A5').edges.length===0,'Rank labels must not appear as a serial physical path');
